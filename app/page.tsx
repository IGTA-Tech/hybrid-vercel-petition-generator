...
export default function Home() {
    ...
    for (const file of pendingFiles) {
      try {
        ...
        if (response.ok) {
          ...
          file.extractedText = data.processed.extractedText;
          ...
          const uploadedFileData: UploadedFileData = {
            ...
            extractedText: data.processed.extractedText, // Store full text for autofill
          };
  ...
  const handleAutoFill = async () => {
    if (uploadedFiles.length === 0) {
      alert('Please upload documents first (Step 4) before using auto-fill.');
      return;
    }
    ...
    try {
      ...
      if (response.ok && data.success) {
        setFormData(prev => ({
          ...prev,
          petitionerName: data.petitionerName || prev.petitionerName,
          itinerary: data.itinerary || prev.itinerary,
          additionalInfo: data.additionalInfo || prev.additionalInfo,
        }));
        alert('✓ Form fields auto-filled from uploaded documents!');
      } else {
        alert('Error auto-filling: ' + (data.error || 'Unknown error'));
      }
    } catch (error) {
      alert('Error auto-filling: ' + error);
    } finally {
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                {uploadedFiles.length > 0 && (
                  ...
                  >
                    <Sparkles size={18} />
                    {autofillLoading ? 'Analyzing Documents...' : 'AI Auto-Fill from Documents'}
                  </button>
                )}
              </div>
              ...
              {uploadedFiles.length > 0 && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <p className="text-sm text-blue-800">
                    💡 <strong>Tip:</strong> Click "AI Auto-Fill from Documents" to automatically extract petitioner name, itinerary, and additional information from your uploaded documents!
                  </p>
                </div>
              )}
